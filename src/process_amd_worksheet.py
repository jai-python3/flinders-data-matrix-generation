...
def process_amd_worksheet(sheet_name: str, worksheet, outdir: str) -> None:
    ...
    Allowed diagnosis values are:
    - AMD
    - CNV only
    - GA only / No active CNV
    ...
    for row in worksheet:
        ...
        for cell in row:
            ...
            #     )
            ...
            elif column_name == "Age.Exam":
                quantitative_id_lookup[current_sample_id]["age_exam"] = cell_value
            elif column_name == "Gender":
                if cell_value.lower().startswith("m"):
                    binary_id_lookup[current_sample_id]["gender"] = MATRIX_GENDER_MALE
                elif cell_value.lower().startswith("f"):
                    binary_id_lookup[current_sample_id]["gender"] = MATRIX_GENDER_FEMALE
                else:
                    print_red(f"Encountered unexpected Gender value '{cell_value}'")
            elif column_name.strip() == "Ancestry":
                if cell_value.lower() == "caucasian":
                    binary_id_lookup[current_sample_id][
                        "ancestry_caucasian"
                    ] = MATRIX_CASE_VALUE
                else:
            ...
            elif column_name == "Diagnosis":
                if cell_value.lower() == "amd":
                    binary_id_lookup[current_sample_id][
                        "diagnosis_amd"
                    ] = MATRIX_CASE_VALUE
                    binary_id_lookup[current_sample_id][
                    ...
                    binary_id_lookup[current_sample_id][
                        "diagnosis_ga_only_no_active_cnv"
                    ] = MATRIX_NA_VALUE
                elif cell_value.lower() == "cnv only":
                    binary_id_lookup[current_sample_id][
                        "diagnosis_amd"
                    ] = MATRIX_NA_VALUE
                    binary_id_lookup[current_sample_id][
                ...
                elif cell_value.lower() == "ga only / no active cnv":
                    binary_id_lookup[current_sample_id][
                        "diagnosis_amd"
                    ] = MATRIX_NA_VALUE
                    binary_id_lookup[current_sample_id][
                    ...
                    print_red(f"Encountered unexpected Diagnosis value '{cell_value}'")
            else:
...
def main(
    ...
    for sheet_name in workbook.sheetnames:
        ...
        if sheet_name == AMD_WORKSHEET_NAME:
            process_amd_worksheet(sheet_name, workbook[sheet_name], outdir)
        else:
            logger.warning(f"Will not process worksheet named '{sheet_name}'")

    # if AMD_WORKSHEET_NAME not in workbook:
    #     print_red(
    #         f"Could not find worksheet '{AMD_WORKSHEET_NAME}' in Excel file '{infile}'"
    #     )
    #     sys.exit(1)

    # process_amd_worksheet(AMD_WORKSHEET_NAME, workbook[AMD_WORKSHEET_NAME], outdir)
